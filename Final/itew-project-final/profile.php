...
$sql = "SELECT firstname, middlename, lastname, age, email, homeaddress FROM user_profile WHERE email = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $update_sql = "UPDATE user_profile SET firstname = ?, middlename = ?, lastname = ?, age = ?, email = ?, homeaddress = ? WHERE username = ?";
    $update_stmt = $conn->prepare($update_sql);

    if ($update_stmt) {
        $update_stmt->bind_param("ssissss", $firstname, $middlename, $lastname, $age, $email, $homeaddress, $username);
